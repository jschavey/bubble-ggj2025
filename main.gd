...
func spawn_fish():
	...
	var fish_size = fish_instance.get_node('Area2D').fish_size.x
	var screen_size = get_viewport().size
	var valid_position = false

	fish_instance.position = Vector2(randf_range(0, screen_size.x), 0)
...
func reset_game():
	...
	for child in get_children():
		if child.name.begins_with("Obstacle") or child.name.begins_with("Fish"):
			child.queue_free()
